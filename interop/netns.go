...
const (
	...
	envNSChild = "BFD_INTEROP_NETNS_CHILD"
	envNSInit  = "BFD_INTEROP_NETNS_INIT"
	envNSRun   = "BFD_INTEROP_NETNS_RUN"
)
...
type nsRuntime struct {
	daemons string // the $BFD_INTEROP_FRR daemon directory
	vtysh   string // the resolved vtysh binary
}

// newNSRuntime validates $BFD_INTEROP_FRR inside the namespaced child.
// Failures are fatal to the process, never a skip: see the package
// documentation.
func newNSRuntime() *nsRuntime {
	dir := os.Getenv(envFRR)
	for _, d := range []string{"zebra", "bfdd"} {
	...
	// never a quiet swap.
	out, err := exec.Command(filepath.Join(dir, "bfdd"), "--version").Output()
	if err != nil {
...
func (n *nsRuntime) startFRR(t *testing.T, name string, conf []byte) *nsFRR {
	...
	run, err := os.MkdirTemp("/tmp", "bfd-interop")
	if err != nil {
	...
	t.Cleanup(func() {
		...
		for _, f := range []string{"zebra.log", "bfdd.log", "init.log"} {
	...
	for {
		...
		_, berr := os.Stat(filepath.Join(run, "bfdd.vty"))
...
func nsInit() {
	...
	// The instance environment: a pinned kernel hostname for stable
	// logs, and private tmpfs over /run and /var/lib so FRR's
	// compiled-in state paths are writable. The bind-mounted group
	// file satisfies zebra's frrvty lookup: see startFRR.
	...
	// Both daemons are required: bfdd learns interface addresses
	// through zebra, like bgpd before it. Neither runs as the missing
	// frr user; the namespace's root plus the group file sidestep the
	// runas dance, exactly as proven for zebra in the bgp suite.
	zebra := daemon("zebra", "-u", "root", "-g", "root")
	bfdd := daemon("bfdd", "-u", "root", "-g", "root")
