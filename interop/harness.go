...
//
// When $BFD_INTEROP_LOGDIR is set, each FRR instance's logs are saved
...
// binary's namespace to the oracle's. The network itself uses RFC 1918
// and ULA space; being one hop apart on a veth pair is what makes the
// RFC 5881 GTSM checks honest in both directions.
const (
...
func instanceName(t *testing.T) string {
	...
	return fmt.Sprintf("bfd-interop-%s-%d", name, os.Getpid())
}

// saveLogs implements the log contract for one FRR instance: logs land
// in $BFD_INTEROP_LOGDIR when it is set, and in the test output when
...
func saveLogs(t *testing.T, name string, logs []byte) {
	...
	if dir := os.Getenv("BFD_INTEROP_LOGDIR"); dir != "" {
